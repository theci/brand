...
>
> 🖥️ **터미널 없이 화면(UI)만 쓸 분께 배포하려면 [`UI_사용_가이드.md`](docs/UI_사용_가이드.md)** — 코딩을 몰라도 사이드바 페이지 순서대로 따라 하도록 쓴 **비개발자용 배포 문서**입니다.
> 📘 **처음이라면 [`GUIDE.md`](docs/가이드.md)를 보세요** — 설치·기능별 사용법·테스트·자주 하는 작업까지 단계별로 정리한 상세 가이드입니다.
> 🔰 **YAML·코딩이 처음이라면 [`데이터_작성_가이드.md`](docs/데이터_작성_가이드.md)** — 데이터 파일을 왜 이렇게 쓰는지, 비전공 신입도 직접 고치고 테스트할 수 있게 설명합니다.
> 🧪 **전체 흐름을 한 제품으로 따라가려면 [`예시_시나리오.md`](docs/예시_시나리오.md)** — 고체 샴푸바 하나를 기획→처방→라벨→원가→안정성→검증까지 실제 명령·출력으로 보여줍니다.
> 📌 **실제로 쓰기 전에 [`사용_참고사항.md`](docs/사용_참고사항.md)도 읽으세요** — 교체해야 할 예시값, 계산 가정, 레짐별 비용 차이를 정리해 두었습니다.
...
제형을 실제로 만들고 개선하는 전 과정을 돕는 명령들입니다. 전체 흐름 예시는 [`예시_시나리오_처방개선.md`](docs/예시_시나리오_처방개선.md).
